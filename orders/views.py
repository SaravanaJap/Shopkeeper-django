...
def payments(request):
     ...
     #send order number and transaction id to senddata method via json response
...
def place_order(request,quantity = 0,total = 0):
    ...
    if request.method == 'POST':
        ...
        print("form page")
        #Storin all billing information inside order table
        if form.is_valid():
                print(form.errors)
                data = Orders()
                ...
                data.tax = tax
                data.ip = request.META.get('REMOTE_ADDR')  #to get user IP
                data.save()
                ...
                order = Orders.objects.get(is_ordered = False,order_number = order_number)
                context = {
                     'order':order,
             ...
             print(form.errors)
             return HttpResponse("ïnvalid form")
    else:
